...
class OnboardingWizard:
    ...
    async def _finish(self, chat_id: str) -> None:
        ...
        text = (
            ...
            f"Режим: <code>ТЕСТ</code> (сообщения НЕ отправляются)\n"
            f"Сообщение: <code>{msg_preview}</code>\n\n"
            f"Товары:\n{products_text}\n\n"
            "━━━━━━━━━━━━━━━━━━\n"
            "<b>Как работает бот:</b>\n\n"
            "— Бот отслеживает НОВЫЕ негативные чаты с момента подключения\n"
            "— Старые чаты (до подключения) не обрабатываются\n"
            "— На паузе чаты копятся, после включения бот обработает накопленные\n"
            "— В режиме ТЕСТ сообщения не отправляются, только уведомления\n"
            "━━━━━━━━━━━━━━━━━━\n\n"
            "Когда будете готовы к боевому режиму — нажмите «Настройки» -> «Режим»."
        )
